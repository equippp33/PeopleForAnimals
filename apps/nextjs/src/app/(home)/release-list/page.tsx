...
function ReleaseListContent() {
    const searchParams = useSearchParams();
    const rangeParam = searchParams.get("range") as Range;
    const dateParam = searchParams.get("date");

    const { data: dogs, isLoading } = api.task.getReleasedDogs.useQuery();
    ...
    const dogRecords = useMemo(() => {
        ...
        return all.filter((dog: any) => {
            const released = dog.releaseDate ? new Date(dog.releaseDate) : dog.release_date ? new Date(dog.release_date) : null;
            if (!released) return false;
            if (released < start) return false;
    ...
    useEffect(() => {
        if (selectedRecords.length !== dogRecords.length) {
            setSelectedRecords(new Array(dogRecords.length).fill(false));
        }
    }, [dogRecords.length, selectedRecords.length]);
    ...
    const handleDownloadExcel = () => {
        const rows = dogRecords.map((dog: any) => ({
            ...
            "Release Date": dog.releaseDate
                ? new Date(dog.releaseDate).toLocaleDateString()
                : dog.release_date
                    ? new Date(dog.release_date).toLocaleDateString()
                    : "-",
            "Location": dog.location || "-",
            "Gender": dog.gender || "-",
            "Status": dog.releaseStatus || dog.release_status || "released",
        }));
        const ws = XLSX.utils.json_to_sheet(rows);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Released Dogs");
        XLSX.writeFile(wb, `released-dogs-${filterLabel.toLowerCase().replace(/\s+/g, "-")}.xlsx`);
    };

    if (isLoading) {
        return <div className="p-8">Loading released dogs...</div>;
    }
    ...
    return (
        <div className="mx-auto max-w-[1800px] p-4 sm:p-6 lg:p-8">
            ...
            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold">Release List</h1>
                    <p className="text-sm text-gray-600">
                        {rangeParam || dateParam
                            ? `Showing releases for: ${filterLabel}`
                            : "All released dogs"}
                    </p>
                </div>
                <button
            ...
            <div className="overflow-hidden rounded-lg border bg-white">
                <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
                    <h2 className="text-center text-base font-semibold sm:text-left sm:text-lg">
                        Release List - {filterLabel === "All Dogs" ? "All Dogs" : filterLabel}
                    </h2>
                </div>
                                ...
                                <tr key={dog.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-2 py-4 align-top sm:px-4">
                                        <span className="rounded-full bg-blue-100 px-2 py-1 text-xs text-blue-800">
                                            {dog.releaseStatus || dog.release_status || "released"}
                                        </span>
                                    </td>
                                </tr>
                            ))}
